...
public class BikeCountPerHourAndTemp implements Serializable {
    private static final Logger LOGGER = LoggerFactory.getLogger(BikeCountPerHourAndTemp.class);

    public static void main(String[] args) {
        checkArgument(args.length > 1, "Please provide the path of input file and output dir as parameters.");
        new BikeCountPerHourAndTemp().run(args[0], args[1]);
    }

    public void run(String inputFilePath, String outputDir) {

        SparkConf conf = new SparkConf()
                .setAppName(BikeCountPerHourAndTemp.class.getName())
                .setMaster("local[*]");
        ...
        // Map each line to a pair of ((hour, temperature category), (count, 1))
        JavaPairRDD<Tuple2<String, String>, Tuple2<Integer, Integer>> counts = data
                .mapToPair(line -> {
                    ...
                    String temperatureCategory = getTemperatureCategory(Double.parseDouble(parts[2]));
                    return new Tuple2<>(new Tuple2<>(hour, temperatureCategory), new Tuple2<>(count, 1));
                })
