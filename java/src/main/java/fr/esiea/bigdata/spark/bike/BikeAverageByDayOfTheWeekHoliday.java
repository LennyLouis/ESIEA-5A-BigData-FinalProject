...
public class BikeAverageByDayOfTheWeekHoliday {
    private static final Logger LOGGER = LoggerFactory.getLogger(BikeAverageByDayOfTheWeekHoliday.class);

    public static void main(String[] args) {
        checkArgument(args.length > 1, "Please provide the path of input file and output dir as parameters.");
        new BikeAverageByDayOfTheWeekHoliday().run(args[0], args[1]);
    }

    public void run(String inputFilePath, String outputDir) {

        SparkConf conf = new SparkConf()
                .setAppName(BikeAverageByDayOfTheWeekHoliday.class.getName())
                .setMaster("local[*]");
        ...
        // Map each line to a pair of ((dayOfWeek, is_holiday), count)
        JavaPairRDD<Tuple2<String, String>, Tuple2<Integer, Integer>> counts = data
                .mapToPair(line -> {
                    ...
                    String isHoliday = parts[7];
                    int count = Integer.parseInt(parts[1]);
                    return new Tuple2<>(new Tuple2<>(dayOfWeek, isHoliday), new Tuple2<>(count, 1));
                })
